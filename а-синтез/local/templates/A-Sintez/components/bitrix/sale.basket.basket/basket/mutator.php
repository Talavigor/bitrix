...
foreach ($this->basketItems as $row)
{
	...
	$rowData = array(
		...
		'NAME_EN' => isset($row['~PROPERTY_CATALOGTITLE_VALUE']) ? $row['~PROPERTY_CATALOGTITLE_VALUE'] : $row['NAME'],
	...
	elseif (!empty($row['DETAIL_PICTURE_SRC']))
	{
		...
		$rowData['IMAGE_URL'] = $row['DETAIL_PICTURE_SRC_2X'];
	}
...
//						'VALUE' => $row['PREVIEW_TEXT'],
			...
			elseif (!empty($row[$value['id']]) && $value['id'] != 'PROPERTY_CATALOGTITLE_VALUE')
			{
                $arName = explode("@", $value['name']);
