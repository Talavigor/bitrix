	...
	BX.Sale.OrderAjaxComponent = {
		...
		createBasketItemImg: function(data)
		{
			...
			logoNode = BX.create('DIV'
				// , {props: {className: 'bx-soa-item-imgcontainer'}} фото товаров в корзине
				);
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			var data = allData.columns[column.id] ? allData.columns : allData.data,
				toRight = BX.util.in_array(column.id, ["QUANTITY", "PRICE_FORMATED", "DISCOUNT_PRICE_PERCENT_FORMATED", "SUM"]),
				textNode = BX.create('DIV', {props: {className: 'basket__quantity bx-soa-item-td-text'}}),
				logotype, img;
		...
		getPersonTypeControl: function(node)
		{
			...
			if (personTypesCount > 1)
			{
				input = BX.create('DIV', {
					...
					children: [
						BX.create('LABEL', {props: {className: 'bx-soa-custom-label'},
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++)
			{
				var idi = inputs[i].getAttribute('id');
                switch(idi) {
                    case 'soa-property-1':
                        inputs[i].setAttribute('placeholder', 'Ваше имя');
                        BX.addClass(inputs[i], 'popup-input__name popup-input form-control bx-soa-customer-input bx-ios-fix');
                        break;

                    case 'soa-property-27':
                        inputs[i].setAttribute('placeholder', 'Ваш Email');
                        BX.addClass(inputs[i], 'popup-input__name popup-input form-control bx-soa-customer-input bx-ios-fix');
                        break;
                    case 'soa-property-3':
                        inputs[i].setAttribute('placeholder', 'Ваш телефон');
                        BX.addClass(inputs[i], 'popup-input__name popup-input form-control bx-soa-customer-input bx-ios-fix');
                        break;

                    default:
                        inputs[i].setAttribute('placeholder', settings.DESCRIPTION);
                        BX.addClass(inputs[i], 'popup-input__name popup-input form-control bx-soa-customer-input bx-ios-fix');
                        break;
                }
		...
		editPropsComment: function(propsNode)
		{
			...
			input = BX.create('TEXTAREA', {
				props: {
					...
					className: 'order__textarea form-control bx-soa-customer-textarea bx-ios-fix',
