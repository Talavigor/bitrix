 ...
 */
$renderImage = CFile::ResizeImageGet($item["DETAIL_PICTURE"]['ID'], Array("width" => 300, "height" => 213), BX_RESIZE_IMAGE_EXACT, false);


$top_lists = CIBlockSection::GetList(Array(SORT => "ASC"),
    $arFilter = Array("IBLOCK_ID" => 11, "ID" => $item['IBLOCK_SECTION_ID']),
    true,
    $arSel = Array("*"));
$top_result = $top_lists->GetNext();
//$top_image = CFile::GetPath($top_result['PICTURE']);
$topImage = CFile::ResizeImageGet($top_result["PICTURE"], Array("width" => 300, "height" => 213), BX_RESIZE_IMAGE_EXACT, false);
?>

<a href="<?= $top_result['SECTION_PAGE_URL'] ?>" title="<?= $top_result['NAME'] ?>"
   data-entity="image-wrapper">


    <div class="img__wr" id="<?= $itemIds['PICT'] ?>">
        <img alt="<?= $item['DETAIL_PICTURE']['ALT'] ?>" src="<?= $topImage['src'] ?>"
             class="recently__item-img">
        <div class="hover-bg">
        </div>
    </div>
...
<a href="<?= $top_result['SECTION_PAGE_URL'] ?>" title="<?= $productTitle ?>">
    <? if (LANGUAGE_ID == 'en' && !empty($item['PROPERTIES']['CATALOGTITLE']['VALUE'])){ ?>
    <span class="recently__item-name"><?= $item['PROPERTIES']['CATALOGTITLE']['VALUE'] ?></span></a>
<? } else { ?>
    <span class="recently__item-name"><?= $productTitle ?></span></a>
<? } ?>
<?
