...
<?if($arResult["ID"]):?>
    ...
    <div id="ask_block_content_in" class="hidden">
        <?$APPLICATION->IncludeComponent(
            "bitrix:form.result.new",
            "popup",
            array(
    ...
    <script type="text/javascript">
        if($("#ask_block_content_in").length && $("#ask_block_in").length){
            $("#ask_block_content_in").appendTo($("#ask_block_in"));
            $("#ask_block_content_in").removeClass("hidden");
        }
        if($(".gifts").length && $("#reviews_content").length){
            $(".gifts").insertAfter($("#reviews_content"));
        }
...
<?if($_REQUEST && isset($_REQUEST['formresult'])):?>
    ...
    <script>
        $(document).ready(function() {
            if($('#ask_block_in .form_result').length){
                $('.product_ask_tab').trigger('click');
            }
        });
    </script>
